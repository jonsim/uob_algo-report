...
the equations found in \ref{sec:AA_price_estimation} (an example can be seen in
Equation \ref{eqn:reverse_intra_buyer} for calculating the reverse of the
intra-marginal buyer).  Since we wish to be more competitive than the best
outstanding shout, the second component measures how much more (or less
aggressive) we will be compared with $r_{shout}$.  $\lambda_r$ and $\lambda_a$
therefore represent the relative and absolute change in $r_{shout}$
respectively. %TODO: Word this better
